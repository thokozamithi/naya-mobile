...
-- Date: 2026-02-13 (v2 - fixed ordering)
-- =============================================
...
drop function if exists public.is_tenant_of_unit(uuid) cascade;
...
-- =============================================
-- STEP 3: CREATE ALL TABLES (without RLS policies yet)
-- =============================================
...
-- Now that tables exist, these can reference them
-- =============================================
...
-- Landlord direct ownership check (no recursion)
create policy "properties_select_owner" on public.properties for select using (auth.uid() = user_id);
create policy "properties_insert_owner" on public.properties for insert with check (auth.uid() = user_id);
create policy "properties_update_owner" on public.properties for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "properties_delete_owner" on public.properties for delete using (auth.uid() = user_id);
-- Tenant can view their property (uses SECURITY DEFINER helper - no recursion)
create policy "properties_select_tenant" on public.properties for select using (public.is_tenant_of_property(id));

-- 5.3 UNITS RLS
alter table public.units enable row level security;
-- Landlord policies (uses SECURITY DEFINER helper)
create policy "units_select_owner" on public.units for select using (public.is_property_owner(property_id));
create policy "units_insert_owner" on public.units for insert with check (public.is_property_owner(property_id));
create policy "units_update_owner" on public.units for update using (public.is_property_owner(property_id));
create policy "units_delete_owner" on public.units for delete using (public.is_property_owner(property_id));
-- Tenant can view their unit (uses SECURITY DEFINER helper)
create policy "units_select_tenant" on public.units for select using (public.is_tenant_of_unit(id));
...
create policy "tenants_update_own" on public.tenants for update using (auth.uid() = user_id) with check (auth.uid() = user_id);
-- Landlord policies (uses SECURITY DEFINER helper to avoid recursion)
create policy "tenants_select_landlord" on public.tenants for select using (public.is_property_owner(property_id));
create policy "tenants_insert_landlord" on public.tenants for insert with check (public.is_property_owner(property_id));
create policy "tenants_update_landlord" on public.tenants for update using (public.is_property_owner(property_id));
create policy "tenants_delete_landlord" on public.tenants for delete using (public.is_property_owner(property_id));

-- 5.5 PAYMENTS RLS
alter table public.payments enable row level security;
create policy "payments_select_own" on public.payments for select using (auth.uid() = user_id);
create policy "payments_insert_own" on public.payments for insert with check (auth.uid() = user_id);
create policy "payments_update_own" on public.payments for update using (auth.uid() = user_id and status = 'pending');
create policy "payments_select_landlord" on public.payments for select using (public.is_property_owner(property_id));
create policy "payments_update_landlord" on public.payments for update using (public.is_property_owner(property_id));
...
create policy "leases_select_tenant" on public.leases for select using (
  exists (select 1 from public.tenants where tenants.id = leases.tenant_id and tenants.user_id = auth.uid())
);
create policy "leases_select_landlord" on public.leases for select using (public.is_property_owner(property_id));
create policy "leases_insert_landlord" on public.leases for insert with check (public.is_property_owner(property_id));
create policy "leases_update_landlord" on public.leases for update using (public.is_property_owner(property_id));
create policy "leases_delete_landlord" on public.leases for delete using (public.is_property_owner(property_id));
...
begin
  ...
  while length(prefix) < 3 loop prefix := prefix || 'X'; end loop;
  for attempt in 1..10 loop
    code := prefix || '-' || lpad(floor(random() * 10000)::text, 4, '0');
    if not exists(select 1 from public.properties where property_code = code) then return code; end if;
  end loop;
...
create trigger trigger_set_property_code before insert on public.properties
...
begin
  loop
    new_code := upper(chr(65 + floor(random() * 26)::int) || chr(65 + floor(random() * 26)::int) || chr(65 + floor(random() * 26)::int) || lpad(floor(random() * 10000)::text, 4, '0'));
    if not exists(select 1 from public.units where unit_join_code = new_code) then return new_code; end if;
  end loop;
end;
$$ language plpgsql;
...
create trigger unit_join_code_trigger before insert or update on public.units
...
begin
  ...
  from public.tenants
  where user_id = v_user_id and property_id = v_unit.prop_id and status = 'active';
...
do $$ begin
  ...
  raise notice '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━';
  raise notice '✅ All tables created: user_roles, properties, units, tenants, payments, messages, leases';
  raise notice '✅ RLS policies use SECURITY DEFINER helpers - NO INFINITE RECURSION';
  raise notice '✅ Property codes auto-generated';
  raise notice '✅ Unit join codes auto-generated';
  raise notice '✅ join_unit_by_code() and leave_unit() RPC functions ready';
  raise notice '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━';
end $$;
