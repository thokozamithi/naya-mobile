...
jest.mock('@/hooks/useAuth', () => ({
  useAuth: () => ({ signOut: jest.fn(), activeRole: 'builder', user: { email: 'test@example.com' } })
}));

const mockNavigate = jest.fn();
const mockNavigation = { navigate: mockNavigate };

describe('BuilderDashboard navigation/buttons', () => {
  it('renders DashboardHeader and quick actions', () => {
    const { getByText } = render(
      <NavigationContainer>
        <BuilderDashboard navigation={mockNavigation} />
      </NavigationContainer>
    );
    expect(getByText('Builder Dashboard')).toBeTruthy();
