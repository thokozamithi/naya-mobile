...
import { useUpdateUnit } from '@/hooks/useData';
import { Unit } from '@/hooks/useData';
...
export default function EditUnitScreen() {
  ...
  const { user } = useAuth();
  const { unit, propertyName } = (route.params as RouteParams) || {};
  ...
  useEffect(() => {
    if (unit) {
      ...
      setBathrooms(unit.bathrooms?.toString() || '1.0');
  ...
  if (!unit) {
    return (
      <View style={styles.container}>
        <Text style={styles.errorTextCentered}>Unit not found</Text>
      </View>
    );
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setUpdatedUnitName(unitName.trim());
  ...
  return (
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {propertyName && (
          <View style={styles.propertyBanner}>
            <Text style={styles.propertyBannerText}>Editing unit in: {propertyName}</Text>
          </View>
        )}
        ...
        <View style={styles.form}>
          ...
          <TouchableOpacity
            style={[styles.submitButton, updateUnit.isPending && styles.submitButtonDisabled]}
      ...
      <SuccessModal
        ...
        title="Unit Updated!"
        message={`${updatedUnitName} has been updated successfully.`}
